ninja.wallets.splitwallet = {
	...
	splitKey: function () {
		try {
			...
			var shares = ninja.wallets.splitwallet.getFormattedShares(key.getMotaCoinHexFormat(), numshares, threshold);

			var output = document.createElement("div");
			output.setAttribute("id", "splitoutput");
			var m = {};
			output.appendChild(this.mkOutputRow(motacoinAddress, "split_addr", "MotaCoin Address:    "));
			m["split_addr"] = motacoinAddress;
	...
	combineShares: function () {
		try {
			...
			var privkeyBase58 = new MotaCoin.ECKey(combinedBytes).getMotaCoinWalletImportFormat();
	...
	getFormattedShares: function (key, numshares, threshold) {
		var shares = secrets.share(key, numshares, threshold).map(ninja.wallets.splitwallet.hexToBytes).map(MotaCoin.Base58.encode);
		return shares;
	},

	// combine base58 formatted shares and return a motacoin byte array
	combineFormattedShares: function (shares) {
		var combined = secrets.combine(shares.map(MotaCoin.Base58.decode).map(Crypto.util.bytesToHex).map(ninja.wallets.splitwallet.stripLeadZeros));
