(function (wallets, qrCode, privateKey, translator) {
	var detail = wallets.detailwallet = {
		...
		checkAndShowMini: function (key) {
			if (MotaCoin.ECKey.isMiniFormat(key)) {
		...
		checkAndShowBase6: function (key) {
			if (MotaCoin.ECKey.isBase6Format(key)) {
			...
			if (btcKey.error != null) {
				...
				if (key.length >= wallets.brainwallet.minPassphraseLength) {
					...
					if (usePassphrase) {
						...
						btcKey = new MotaCoin.ECKey(bytes);
					}
				}
				else {
					alert(translator.get("detailalertnotvalidprivatekey"));
		...
		decryptBip38: function () {
			...
			privateKey.BIP38EncryptedKeyToByteArrayAsync(key, passphrase, function (btcKeyOrError) {
				...
				if (btcKeyOrError.message) {
					...
					detail.populateKeyDetails(new MotaCoin.ECKey(btcKeyOrError));
				}
			});
		},
		...
		encryptBip38: function () {
			...
			if (detailEncryptCheckbox.checked == true) {
				...
				document.getElementById("busyblock").className = "busy";
				privateKey.BIP38PrivateKeyToEncryptedKeyAsync(btcKey.getMotaCoinWalletImportFormat(), passphrase, btcKey.compressed, function (encryptedKey) {
					qrCode.showQrCode({
		...
		populateKeyDetails: function (btcKey) {
			if (btcKey.priv != null) {
				...
				var motacoinAddress = btcKey.getMotaCoinAddress();
				var wif = btcKey.getMotaCoinWalletImportFormat();
				document.getElementById("detailpubkey").innerHTML = btcKey.getPubKeyHex();
				document.getElementById("detailaddress").innerHTML = motacoinAddress;
				document.getElementById("detailprivwif").innerHTML = wif;
				btcKey.setCompressed(true);
				var motacoinAddressComp = btcKey.getMotaCoinAddress();
				var wifComp = btcKey.getMotaCoinWalletImportFormat();			
				document.getElementById("detailpubkeycomp").innerHTML = btcKey.getPubKeyHex();
				document.getElementById("detailaddresscomp").innerHTML = motacoinAddressComp;
				document.getElementById("detailprivwifcomp").innerHTML = wifComp;
				btcKey.setCompressed(originalCompression); // to satisfy the key pool
				var pool1 = new MotaCoin.ECKey(wif); // to satisfy the key pool
				var pool2 = new MotaCoin.ECKey(wifComp); // to satisfy the key pool

				qrCode.showQrCode({
					"detailqrcodepublic": motacoinAddress,
					"detailqrcodepubliccomp": motacoinAddressComp,
