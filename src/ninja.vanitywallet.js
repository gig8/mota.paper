ninja.wallets.vanitywallet = {
	...
	generateKeyPair: function () {
		...
		var privateKey = key.getMotaCoinHexFormat();
	...
	addKeys: function () {
		...
		var motacoinAddress = ninja.translator.get("vanityinvalidinputcouldnotcombinekeys");
		...
		try {
			...
			if (ninja.publicKey.isPublicKeyHexFormat(input1KeyString) && ninja.publicKey.isPublicKeyHexFormat(input2KeyString)) {
				...
				if (document.getElementById("vanityradioadd").checked) {
					...
					if (pubKeyByteArray == null) {
						...
						motacoinAddress = ninja.publicKey.getMotaCoinAddressFromByteArray(pubKeyByteArray);
			...
			// one public key and one private key
				...
				var ecKey = (ninja.privateKey.isPrivateKey(input1KeyString)) ? new MotaCoin.ECKey(input1KeyString) : new MotaCoin.ECKey(input2KeyString);
				// add 
				if (document.getElementById("vanityradioadd").checked) {
				...
				if (pubKeyCombined == null) {
					...
					motacoinAddress = ninja.publicKey.getMotaCoinAddressFromByteArray(pubKeyCombined);
			...
			// both inputs are private keys
				...
				if (combinedPrivateKey == null) {
					...
					motacoinAddress = combinedPrivateKey.getMotaCoinAddress();
					privateKeyWif = combinedPrivateKey.getMotaCoinWalletImportFormat();
		...
		document.getElementById("vanityaddress").innerHTML = motacoinAddress;
